...
subplot(2,2,2)
imshow(dm2.data');title("J'*cumsum*J*dm")
subplot(2,2,3)
...
x1 = 0f0 .* dm
start1 = J'*P*d_lin
x1,his1 = cg!(x1,M'*J'*P*J*M,start1,maxiter=maxit,log=true,verbose=true)
x2 = 0f0 .* dm
start2 = J'*d_lin
x2,his2 = cg!(x2,M'*J'*J*M,start2,maxiter=maxit,log=true,verbose=true)
res1 = his1[:resnorm]
res2 = his2[:resnorm]

u1 = 20*log.(res1./norm(start1))
u2 = 20*log.(res2./norm(start2))

figure();
xlabel("iterations")
ylabel("ratio")
PyPlot.plot(res1/norm(start1),label="CG")
PyPlot.plot(res2/norm(start2),label="P-CG")
...
xlabel("iterations")
ylabel("dB")
PyPlot.plot(u1,label="CG")
PyPlot.plot(u2,label="P-CG")
title("normalized log-based least squares residual")
legend()

figure();
subplot(2,2,1)
imshow(reshape(M*x1,n)');title("CG")
subplot(2,2,2)
imshow(reshape(M*x2,n)');title("P-CG")
subplot(2,2,3)
